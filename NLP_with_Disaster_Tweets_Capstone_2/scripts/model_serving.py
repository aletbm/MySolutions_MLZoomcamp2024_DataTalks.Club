...
def transform_text(tweet):
    ...
    tokenized_text = tokenized_text.astype(np.int64)
    tokenized_keyword = tokenized_keyword.astype(np.int64)
    context = np.expand_dims(context, axis=0).astype(np.int32)
...
def make_request(tokenized_text, tokenized_keyword, context):
    ...
    pb_request.inputs['inputs'].CopyFrom(np_to_protobuf(tokenized_text))
    pb_request.inputs['inputs_1'].CopyFrom(np_to_protobuf(tokenized_keyword))
    pb_request.inputs['inputs_2'].CopyFrom(np_to_protobuf(context))
    return pb_request
